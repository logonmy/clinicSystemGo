...
func TreatmentPatientCreate(ctx iris.Context) {
	...
	var treatmentPatientsetsValues []string
	...
	for index, v := range results {
		...
		orderSn := "F7" + strconv.FormatInt(timestamp, 10) + random
		...
		treatmentPatientsetsValues = append(treatmentPatientsetsValues, tstr)
		mstr := "(" + strings.Join(sm, ",") + ")"
		mzUnpaidOrdersValues = append(mzUnpaidOrdersValues, mstr)
	}
	tSetStr := strings.Join(treatmentPatientSets, ",")
	tValueStr := strings.Join(treatmentPatientsetsValues, ",")
