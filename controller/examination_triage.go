...
// ExaminationTriageList 获取检查记录（包括 待检查，已检查，检查中）
func ExaminationTriageList(ctx iris.Context) {
	...
	where mpo.clinic_triage_patient_id = $1 and mpo.order_status = $2 and mpo.charge_project_type_id = 4`
...
func examinationTriageList(ctx iris.Context, status string) {
	...
	left join (select clinic_triage_patient_id,count(*) as total_count from mz_paid_orders where charge_project_type_id = 4 and order_status = '` + status + `' group by(clinic_triage_patient_id)) up on up.clinic_triage_patient_id = ctp.id 
